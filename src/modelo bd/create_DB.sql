...
CREATE TABLE IF NOT EXISTS `db_magictrip`.`avaliacao_destinos` (
  ...
  `id_avaliacao_destino` INT NOT NULL AUTO_INCREMENT,
  PRIMARY KEY (`id_avaliacao_destino`),
  UNIQUE INDEX `id_avaliacao_destino_UNIQUE` (`id_avaliacao_destino` ASC) VISIBLE,
  INDEX `fk_avalicao_destinos_usuario_idx` (`id_usuario` ASC) VISIBLE,
  INDEX `fk_avaliacao_destino_destino_idx` (`id_destino` ASC) VISIBLE,
  CONSTRAINT `fk_avaliacao_destino_destino`
    FOREIGN KEY (`id_destino`)
    REFERENCES `db_magictrip`.`destinos` (`id_destino`),
  CONSTRAINT `fk_avalicao_destinos_usuario`
...
CREATE TABLE IF NOT EXISTS `db_magictrip`.`avaliacao_vendedor` (
  ...
  `id_usuario` INT NOT NULL,
  `id_avalicao_vendedor` INT NOT NULL AUTO_INCREMENT,
  PRIMARY KEY (`id_avalicao_vendedor`),
  UNIQUE INDEX `id_avalicao_vendedor_UNIQUE` (`id_avalicao_vendedor` ASC) VISIBLE,
  INDEX `id_vendedor` (`id_vendedor` ASC) VISIBLE,
  INDEX `fk_avaliacao_vendedor_usuario_idx` (`id_usuario` ASC) VISIBLE,
  CONSTRAINT `fk_avaliacao_vendedor_usuario`
    FOREIGN KEY (`id_usuario`)
    REFERENCES `db_magictrip`.`usuario` (`id_usuario`),
